const containerForDates = document.querySelector('.dates_container');
const nameOfCurrentMonth = document.getElementById('name_of_month');
let today = new Date();
let offset = 0;
const toNextMonth = document.getElementById('to_next');
const toPreviousMonth = document.getElementById('to_previous');
...
function createCalendar() {
    ...
    for (let i = 1; i <= daysCount; i++) {
        ...
        dates.addEventListener('mouseenter', () => {
...
const weatherPicture = document.querySelector('.forecast_image')
...
function createTaskList(event) {
    ...
    if (elementClicked.className != "calendar_empty_cell") {
        if(elementClicked.className === 'forecast' || elementClicked.className ==='forecast_image'){
            elementClicked = elementClicked.parentElement
        }
        elementClicked.classList.add('cliked_cell');
        
        let uniqueId = elementClicked.childNodes[0].textContent + ' ' + nameOfCurrentMonth.innerText;
        let arr = JSON.parse(localStorage.getItem(uniqueId))
...
loadWeather();
let forecastContainers = document.querySelector('.today_calendar_cell');

function temperatureRender(result) {
    for(let i = 0; i < 3; i++) {
        let degrees = document.createElement('div');
        let forecastImage = document.createElement('img');
        degrees.textContent = `${result[i].temperature}`;
        forecastImage.src = `images/${result[i].weatherIcon}.svg`;
        forecastImage.className = 'forecast_image';
        degrees.className = 'forecast';
        forecastContainers.style.position = 'relative';
        forecastContainers.append(degrees);
        forecastContainers.append(forecastImage);
        forecastContainers = forecastContainers.nextSibling;
    }
}
